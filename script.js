...
const YOUR_API_KEY = "09dc0dc4938ba2ce995d455cdfc8fa24185bba8b5f228dbd41ac04d1595abc5e";
...
const API_BASE = "https://apis.data.go.kr/1613000/ArvlInfoInqireService/getSttnAcctoArvlPrearngeInfoList";
const ROUTE_API_BASE = "https://apis.data.go.kr/1613000/BusRouteInfoInqireService/getRouteNoList";
const ROUTE_STOPS_API_BASE = "https://apis.data.go.kr/1613000/BusRouteInfoInqireService/getRouteAcctoThrghSttnList";
const LOC_API_BASE = "https://apis.data.go.kr/1613000/BusLcInfoInqireService/getRouteAcctoBusLcList";
const STOP_API_BASE = "https://apis.data.go.kr/1613000/BusSttnInfoInqireService/getSttnNoList";
const ODSAY_API_KEY = "kQlAB1GbgaHAAgBczrcMwQ";
...
const ROUTE_TYPE_COLORS = {
  "일반버스": "#16a34a", "지선버스": "#16a34a", "순환버스": "#8b5cf6",
  "좌석버스": "#0a6cff", "간선버스": "#0a6cff",
  "급행버스": "#e0392f", "광역버스": "#e0392f",
  "마을버스": "#f4b400",
  default: "#16a34a",
};
...
function buildUrl() {
  const qs = new URLSearchParams({ serviceKey: YOUR_API_KEY, cityCode: currentCityCode, nodeId: currentNodeId, numOfRows: "100", pageNo: "1", _type: "json" });
...
function parseResponse(raw) {
  ...
  try {
    ...
    if (header && header.resultCode !== "00") return { code: header.resultCode === "03" ? "NO_DATA" : "ERROR", msg: header.resultMsg };
    if (!body || Number(body.totalCount ?? 0) === 0) return { code: "NO_DATA", items: [] };
    let items = body?.items?.item ?? [];
    if (!Array.isArray(items)) items = [items];
    return { code: "OK", items: items.map((it) => normalize(it, pick)) };
  } catch (e) {}
  const doc = new DOMParser().parseFromString(raw, "application/xml");
  if (doc.querySelector("parsererror")) return { code: "ERROR", msg: "XML 파싱 실패" };
  const resultCode = doc.querySelector("resultCode")?.textContent?.trim();
  if (resultCode && resultCode !== "00") return { code: resultCode === "03" ? "NO_DATA" : "ERROR", msg: doc.querySelector("resultMsg")?.textContent };
  const nodes = [...doc.querySelectorAll("item")];
  if (!nodes.length) return { code: "NO_DATA", items: [] };
  return { code: "OK", items: nodes.map((el) => { const o = {}; [...el.children].forEach((c) => { o[c.tagName.toLowerCase()] = c.textContent.trim(); }); return normalize(o, pick); }) };
}

function normalize(it, pick) {
  const lower = {};
  Object.keys(it).forEach((k) => { lower[k.toLowerCase()] = it[k]; });
  let sec = Number(pick(lower, "arrtime", "arrivaltime", "arrtm") ?? NaN);
  if (Number.isNaN(sec)) { const min = Number(pick(lower, "predicttime", "predicttime1", "arrivalmin") ?? NaN); sec = Number.isNaN(min) ? NaN : min * 60; }
  return { routeno: String(pick(lower, "routeno", "routenm", "busnumber") ?? "").trim(), arrtime: Number.isNaN(sec) ? null : sec, prevCnt: Number(pick(lower, "arrprevstationcnt", "prevstationcnt", "locationno") ?? 0) };
}
...
function loadFavorites() {
  try {
    ...
    if (oldRaw) {
      ...
      if (Array.isArray(oldArr)) return oldArr.map((r) => ({ city: "22", routeno: r }));
    }
    return [];
  } catch (e) { return []; }
}
...
function isFavorite(city, routeno) { return favorites.some((f) => f.city === city && f.routeno === routeno); }
function toggleFavorite(city, routeno) {
  if (isFavorite(city, routeno)) favorites = favorites.filter((f) => !(f.city === city && f.routeno === routeno));
...
function renderFavoritesSidebar() {
  ...
  if (favorites.length === 0) {
    ...
    empty.textContent = "즐겨찾는 노선이 없습니다. 위의 검색이나 도착정보의 별 버튼을 누르면 추가됩니다.";
  ...
  favorites.forEach((fav, i) => {
    ...
    status.textContent = (CITY_NAMES[fav.city] || fav.city) + " · 탭하여 노선도 보기";
    ...
    star.title = "즐겨찾기 해제";
    star.addEventListener("click", (e) => { e.stopPropagation(); toggleFavorite(fav.city, fav.routeno); });
...
function renderRouteResults(items) {
  ...
  unique.slice(0, 24).forEach((r, i) => {
    ...
    star.className = "material-symbols-outlined route-result-star" + (fav ? " icon-fill is-fav" : "");
    star.textContent = FAV_ICON;
...
async function searchRoutes(query) {
  const box = document.getElementById("routeSearchResults");
  if (!query || query.trim().length === 0) { if (box) box.innerHTML = ""; return; }
  if (box) {
    box.innerHTML = "";
  ...
  try {
    const results = await Promise.all(CITY_CODES.map(async (city) => {
      const qs = new URLSearchParams({ serviceKey: YOUR_API_KEY, cityCode: city, routeNo: query.trim(), numOfRows: "30", pageNo: "1", _type: "json" });
...
function buildPopupList(entries) {
  ...
  entries.forEach((it, i) => {
    ...
    const badge = document.createElement("span");
    badge.className = "badge clickable " + BADGE_COLORS[i % BADGE_COLORS.length];
    badge.textContent = it.routeno;
    badge.addEventListener("click", () => openRouteDiagram(cityAtPopup, it.routeno));
    ...
    star.className = "material-symbols-outlined popup-star" + (fav ? " icon-fill is-fav" : "");
    star.textContent = FAV_ICON;
    star.addEventListener("click", (e) => {
      e.stopPropagation();
...
function selectStop(stop) {
  ...
  if (cityEl) cityEl.textContent = CITY_NAMES[currentCityCode] || currentCityCode;
  const input = document.getElementById("stopSearchInput");
  if (input) input.value = "";
  const box = document.getElementById("stopSearchResults");
  if (box) box.innerHTML = "";
  liveBusMarkers.forEach((entry) => { clearBusAnim(entry); entry.overlay.setMap(null); });
  liveBusMarkers.clear();
  if (stop.gpslati && stop.gpslong) {
    const lat = Number(stop.gpslati), lng = Number(stop.gpslong);
    if (window.__map) {
...
async function setInitialStop() {
  try {
    const qs = new URLSearchParams({ serviceKey: YOUR_API_KEY, cityCode: DEFAULT_STOP_CITY, nodeNm: DEFAULT_STOP_NAME, numOfRows: "5", pageNo: "1", _type: "json" });
    const json = await fetchJson(`${STOP_API_BASE}?${qs.toString()}`);
    const items = asArray(json?.response?.body?.items?.item);
    if (items.length > 0) {
      const stop = Object.assign({}, items[0], { citycode: items[0].citycode || DEFAULT_STOP_CITY });
      selectStop(stop);
      return true;
    }
  } catch (e) {
...
async function searchStops(query) {
  ...
  try {
    const results = await Promise.all(CITY_CODES.map(async (city) => {
      const qs = new URLSearchParams({ serviceKey: YOUR_API_KEY, cityCode: city, nodeNm: query.trim(), numOfRows: "20", pageNo: "1", _type: "json" });
      const json = await fetchJson(`${STOP_API_BASE}?${qs.toString()}`);
      const items = asArray(json?.response?.body?.items?.item);
      return items.map((it) => Object.assign({}, it, { citycode: it.citycode || city }));
    }));
  ...
  gumiBisPromise = (async () => {
    try {
      const [colorRes, routeRes] = await Promise.all([
        fetch(GUMI_BIS_BASE + "/selectBusColorList", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: "" }),
        fetch(GUMI_BIS_BASE + "/selectRealtimeRouteList", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: "" }),
      ]);
      if (!colorRes.ok || !routeRes.ok) throw new Error("bad status");
      const colorJson = await colorRes.json();
      const routeJson = await routeRes.json();
      gumiPlateMap = new Map();
      (colorJson.rows || []).forEach((r) => { if (r.busStandardid) gumiPlateMap.set(String(r.busStandardid), r.busNo); });
...
async function fetchGumiUnofficial(routeno) {
  let ok = false;
  try { ok = await ensureGumiBisData(); } catch (e) { ok = false; }
  if (!ok) return [];
  ...
  try {
    const results = await Promise.all(routeIds.map(async (rid) => {
      ...
      const res = await fetch(GUMI_BIS_BASE + "/getRealtimeBusLoc", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: params.toString() });
      if (!res.ok) return [];
...
async function getRouteMeta(city, routeno) {
  ...
  try {
    const qs = new URLSearchParams({ serviceKey: YOUR_API_KEY, cityCode: city, routeNo: routeno, numOfRows: "10", pageNo: "1", _type: "json" });
    const json = await fetchJson(`${ROUTE_API_BASE}?${qs.toString()}`);
    const items = asArray(json?.response?.body?.items?.item);
    const match = items.find((it) => String(it.routeno) === String(routeno)) || items[0];
    const meta = { routeId: match ? match.routeid : null, routetp: match ? (match.routetp || match.routetCd || match.routeType || "") : "" };
...
async function getRouteStopsList(city, routeno, routeId) {
  ...
  try {
    const qs = new URLSearchParams({ serviceKey: YOUR_API_KEY, cityCode: city, routeId: routeId, numOfRows: "100", pageNo: "1", _type: "json" });
    const json = await fetchJson(`${ROUTE_STOPS_API_BASE}?${qs.toString()}`);
    let items = asArray(json?.response?.body?.items?.item);
    items.sort(function (a, b) { return Number(a.nodeord) - Number(b.nodeord); });
...
function showRouteDiagramModal(city, routeno, opts) {
  ...
  if (!overlay) {
    ...
    overlay.addEventListener("click", function (e) { if (e.target === overlay) closeRouteDiagram(); });
  ...
  overlay.innerHTML =
    '<section class="win route-diagram-window">' +
      '<div class="win-titlebar">' +
        ...
        '<span class="win-title">' + cityName + ' ' + routeno + ' 노선도</span>' +
      '</div>' +
      '<div class="route-diagram-body">' +
        '<div class="route-diagram-head">' +
          '<span class="badge b-blue" style="font-size:14px;min-width:44px;height:32px;">' + routeno + '</span>' +
          '<span class="rd-live">' + (opts.liveText || '') + '</span>' +
          '<span class="material-symbols-outlined rd-fav-btn ' + (fav ? "icon-fill is-fav" : "") + '" id="rdFavBtn">' + FAV_ICON + '</span>' +
        '</div>' +
        '<div class="route-diagram-list" id="rdList"></div>' +
      '</div>' +
    '</section>';

  document.getElementById("rdClose").addEventListener("click", closeRouteDiagram);
  ...
  if (opts.loading) { listEl.innerHTML = '<div class="rd-loading">불러오는 중…</div>'; return; }
  if (opts.errorMsg) { listEl.innerHTML = '<div class="rd-loading">' + opts.errorMsg + '</div>'; return; }
  const stops = opts.stops || [];
  if (stops.length === 0) { listEl.innerHTML = '<div class="rd-loading">정류소 정보가 없습니다</div>'; return; }
  ...
  stops.forEach(function (s, idx) {
    ...
    row.innerHTML =
      ...
      '<div class="rd-stop-info">' +
        '<span class="rd-stop-name">' + (s.nodenm || "") + '</span>' +
        '<span class="rd-stop-id">' + (s.nodeno || s.nodeid || "") + '</span>' +
      '</div>' +
      (isActive ? '<span class="material-symbols-outlined icon-fill rd-stop-bus-icon" title="현재 버스 위치">directions_bus</span>' : '');
...
async function openRouteDiagram(city, routeno) {
  ...
  if (!routeId) { showRouteDiagramModal(city, routeno, { errorMsg: "노선 정보를 찾을 수 없습니다" }); return; }

  const stops = await getRouteStopsList(city, routeno, routeId);
  if (!stops || stops.length === 0) {
    showRouteDiagramModal(city, routeno, { errorMsg: "노선도를 불러오지 못했습니다" });
    return;
  }

  let positions = [];
  try { positions = await fetchBusPositions(city, routeno); } catch (e) {}
  ...
  const liveText = positions.length > 0 ? ("현재 " + positions.length + "대 운행중") : "운행 정보 없음";
  showRouteDiagramModal(city, routeno, { stops: stops, liveText: liveText, busStopMap: busStopMap });
}
...
function getPlacesService() {
  if (!kakaoPlacesSvc && window.kakao && window.kakao.maps && window.kakao.maps.services) {
...
function searchPlacesKakao(query, cb) {
  ...
  svc.keywordSearch(query, function (data, status) {
    if (status === kakao.maps.services.Status.OK) {
      cb(data.slice(0, 8).map((d) => ({ name: d.place_name, addr: d.road_address_name || d.address_name, lng: Number(d.x), lat: Number(d.y) })));
    } else {
      cb([]);
    }
  });
...
function renderDirSuggestions(boxId, items, onSelect) {
  ...
  items.forEach((it) => {
    ...
    row.innerHTML = it.name + '<span class="dir-sugg-addr">' + (it.addr || "") + "</span>";
...
function selectDirPlace(which, place) {
  if (which === "start") dirStart = place; else dirEnd = place;
  const input = document.getElementById(which === "start" ? "dirStartInput" : "dirEndInput");
  if (input) input.value = place.name;
  const box = document.getElementById(which === "start" ? "dirStartSuggest" : "dirEndSuggest");
  if (box) box.innerHTML = "";
  updateDirSearchBtn();
}

function dirTrafficLabel(sub) {
  if (sub.trafficType === 3) return { type: "walk", text: "도보 " + Math.round(sub.distance || 0) + "m" };
  if (sub.trafficType === 2) {
    const bus = (sub.lane && sub.lane[0] && sub.lane[0].busNo) || "버스";
    return { type: "bus", text: bus, sub: (sub.stationCount ? sub.stationCount + "정거장 이동" : "") };
  }
  if (sub.trafficType === 1) {
    const line = (sub.lane && sub.lane[0] && sub.lane[0].name) || "지하철";
    return { type: "subway", text: line, sub: (sub.stationCount ? sub.stationCount + "정거장 이동" : "") };
  }
  return { type: "walk", text: "이동" };
}

function clearDirRoutePath() {
  dirPathOverlays.forEach(function (o) { o.setMap(null); });
  dirPathOverlays = [];
}
...
function createSimplePin(map, position, hexColor, iconName) {
  ...
  el.style.cssText =
    "width:26px;height:26px;border-radius:50% 50% 50% 4px;transform:rotate(45deg);display:flex;align-items:center;justify-content:center;box-shadow:0 3px 8px rgba(0,0,0,0.35);background:" + hexColor + ";";
  el.innerHTML = '<span class="material-symbols-outlined icon-fill" style="transform:rotate(-45deg);color:#fff;width:15px;height:15px;font-size:15px;">' + iconName + "</span>";
  return new kakao.maps.CustomOverlay({ map: map, position: position, content: el, yAnchor: 1, xAnchor: 0.5, clickable: false });
}
...
function drawStartEndMarkers(map, bounds) {
  if (dirStart) {
    ...
    dirPathOverlays.push(createSimplePin(map, startPos, "#0a6cff", "trip_origin"));
  }
  if (dirEnd) {
...
function drawFallbackStraightPath(map, bounds, subPaths) {
  subPaths.forEach(function (sub) {
    ...
    const color = isWalk ? "#9aa3ad" : (sub.trafficType === 1 ? "#16a34a" : "#0a6cff");
    const line = new kakao.maps.Polyline({ path: points, strokeWeight: isWalk ? 4 : 6, strokeColor: color, strokeOpacity: 0.9, strokeStyle: isWalk ? "shortdash" : "solid" });
    line.setMap(map);
...
async function drawDirRoutePath(path) {
  ...
  if (mapObj) {
    try {
      const qs = new URLSearchParams({ apiKey: ODSAY_API_KEY, mapObject: "0:0@" + mapObj });
  ...
  if (laneData && Array.isArray(laneData.lane) && laneData.lane.length > 0) {
    laneData.lane.forEach(function (lane, idx) {
      ...
      const color = isWalk ? "#9aa3ad" : (sub && sub.trafficType === 1 ? "#16a34a" : "#0a6cff");
      (lane.section || []).forEach(function (section) {
        const points = (section.graphPos || []).map(function (p) { return new kakao.maps.LatLng(Number(p.y), Number(p.x)); });
...
function renderDirResults(paths) {
  ...
  paths.slice(0, 5).forEach((p) => {
    ...
    meta.textContent = "환승 " + transfers + "회 · 도보 " + Math.round((info.totalWalk || 0)) + "m";
    const selectedBadge = document.createElement("span");
    selectedBadge.className = "dir-route-selected-badge";
    selectedBadge.innerHTML = '<span class="material-symbols-outlined icon-fill">cancel</span>취소';
    ...
    (p.subPath || []).forEach((sub, idx) => {
      ...
      if (label.type === "walk") {
        ...
        const el = document.createElement("span");
        el.className = "dir-leg-badge " + (label.type === "subway" ? "dir-leg-subway" : "dir-leg-bus");
        el.textContent = label.text;
        el.title = label.text;
        legs.appendChild(el);
      }
...
async function searchTransitRoute() {
  ...
  try {
    const qs = new URLSearchParams({
      ...
      SX: String(dirStart.lng), SY: String(dirStart.lat),
      EX: String(dirEnd.lng), EY: String(dirEnd.lat),
    });
    const json = await fetchJson(`${ODSAY_BASE}?${qs.toString()}`);
    if (json.error) {
      if (box) box.innerHTML = '<div class="rd-loading">' + (json.error[0]?.message || "경로를 찾을 수 없습니다") + '</div>';
      return;
    }
    ...
    if (box) box.innerHTML = '<div class="rd-loading">경로 검색에 실패했습니다</div>';
  }
}
...
function openDirections() {
  ...
  overlay.addEventListener("click", function (e) { if (e.target === overlay) closeDirections(); });
  var shell = document.querySelector(".app-shell");
  (shell || document.body).appendChild(overlay);
  ...
  document.getElementById("dirSearchBtn").addEventListener("click", searchTransitRoute);

  const startInput = document.getElementById("dirStartInput");
  const endInput = document.getElementById("dirEndInput");
  const debouncedStart = debounce((q) => { if (!q) { document.getElementById("dirStartSuggest").innerHTML = ""; return; } searchPlacesKakao(q, (items) => renderDirSuggestions("dirStartSuggest", items, (p) => selectDirPlace("start", p))); }, 300);
  const debouncedEnd = debounce((q) => { if (!q) { document.getElementById("dirEndSuggest").innerHTML = ""; return; } searchPlacesKakao(q, (items) => renderDirSuggestions("dirEndSuggest", items, (p) => selectDirPlace("end", p))); }, 300);
...
async function fetchBusPositions(city, routeno) {
  ...
  if (!routeId) return [];
  try {
    const qs = new URLSearchParams({ serviceKey: YOUR_API_KEY, cityCode: city, routeId: routeId, numOfRows: "50", pageNo: "1", _type: "json" });
    const json = await fetchJson(`${LOC_API_BASE}?${qs.toString()}`);
    const items = asArray(json?.response?.body?.items?.item);
    if (items.length === 0) return [];

    const direct = items
      .map((it) => ({ lat: Number(it.gpslati), lng: Number(it.gpslong), vehicleno: it.vehicleno }))
    ...
    stops.forEach((s) => {
      if (s.nodeid) nodeMap.set(s.nodeid, { lat: Number(s.gpslati), lng: Number(s.gpslong) });
      if (s.nodenm) nameMap.set(String(s.nodenm).trim(), { lat: Number(s.gpslati), lng: Number(s.gpslong) });
    });

    let unofficial = [];
    if (CITY_STOP_BASED[city]) {
      try { unofficial = await fetchGumiUnofficial(routeno); } catch (e) { unofficial = []; }
    }

    return items
      .map((it) => {
        const node = nodeMap.get(it.nodeid);
        if (!node || Number.isNaN(node.lat) || Number.isNaN(node.lng)) return null;
        const pos = { lat: node.lat, lng: node.lng, vehicleno: it.vehicleno, nodeid: it.nodeid };
        if (unofficial.length > 0) {
          const match = unofficial.find((u) => u.plate === it.vehicleno);
          if (match && match.remainTimeSec > 0) {
            const target = nameMap.get(String(match.targetStopName || "").trim());
...
function createBusOverlay(map, position, hexColor, routeno, city, headingDeg) {
  ...
  const el = document.createElement("div");
  el.style.cssText = "position:relative;width:28px;height:42px;cursor:pointer;pointer-events:auto;";
  el.innerHTML =
    '<div class="bus-rot" style="width:28px;height:42px;transform:rotate(' + (headingDeg || 0) + 'deg);transition:transform 0.5s ease;filter:drop-shadow(0 3px 6px rgba(0,0,0,0.35));">' +
      '<svg width="28" height="42" viewBox="0 0 28 42">' +
        '<rect x="2" y="2" width="24" height="38" rx="12" fill="' + hexColor + '" stroke="#fff" stroke-width="1.6"/>' +
        '<line x1="6" y1="11" x2="22" y2="11" stroke="#fff" stroke-width="1.6"/>' +
        '<text x="14" y="27" text-anchor="middle" fill="#fff" font-weight="800" font-size="' + fontSize + '" font-family="Pretendard Variable, Pretendard, sans-serif">' + label + '</text>' +
      '</svg>' +
    '</div>';
...
function clearBusAnim(entry) {
  if (entry && entry.animTimer) { cancelAnimationFrame(entry.animTimer); entry.animTimer = null; }
}
...
async function updateBusMarkers(entries) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    positions.forEach((p, idx) => {
      const key = p.vehicleno ? "v:" + p.vehicleno : "r:" + it.routeno + ":" + idx;
      seenKeys.add(key);
      ...
      if (existing) {
        ...
        if (Math.abs(existing.lastLat - targetLat) > 1e-7 || Math.abs(existing.lastLng - targetLng) > 1e-7) {
          const brng = computeBearing(existing.lastLat, existing.lastLng, targetLat, targetLng);
          if (existing.rotEl) existing.rotEl.style.transform = "rotate(" + brng.toFixed(1) + "deg)";
        }
        if (p.animTo) {
          animateBusTo(existing, p.lat, p.lng, p.animTo.lat, p.animTo.lng, p.animMs);
          ...
          if (Math.abs(curLat - p.lat) > 1e-7 || Math.abs(curLng - p.lng) > 1e-7) {
            animateBusTo(existing, curLat, curLng, p.lat, p.lng, DEFAULT_GLIDE_MS);
          } else {
        ...
        const created = createBusOverlay(map, pos, hexColor, it.routeno, cityAtUpdate, 0);
        const entry = {
        ...
        if (p.animTo) {
          animateBusTo(entry, p.lat, p.lng, p.animTo.lat, p.animTo.lng, p.animMs);
        }
      }
    });
...
async function loadBusArrivals() {
  if (!YOUR_API_KEY) { showPopupMessage("API 키 미설정"); return; }
  try {
    const res = await fetch(buildUrl(), { headers: { Accept: "application/json" } });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const { code, items = [], msg } = parseResponse(await res.text());
    if (code === "ERROR") { showPopupMessage("조회 오류"); return; }
    if (code === "NO_DATA" || items.length === 0) { showPopupMessage("운행 차량 없음"); return; }

    const fastest = new Map();
    items.forEach((it) => {
      if (!it.routeno) return;
      const prev = fastest.get(it.routeno);
      if (!prev || (it.arrtime ?? 1e9) < (prev.arrtime ?? 1e9)) fastest.set(it.routeno, it);
    });
    ...
    showPopupMessage(err.message.includes("fetch") ? "네트워크/CORS 오류" : "정보를 불러올 수 없음");
  }
}
...
function initKakaoMapWhenReady() {
  ...
  var tries = 0;
  (function poll() {
    if (window.kakao && window.kakao.maps) { startKakaoMap(container); return; }
    tries++;
    if (window.__kakaoSdkFailed || tries > 100) {
      console.error("[kakao] SDK 미로드");
      if (container) { var fb = document.createElement("div"); fb.className = "map-fallback"; fb.innerHTML = "지도를 불러오지 못했습니다.<br />API 키와 사이트 도메인 등록을 확인하세요."; container.appendChild(fb); }
      return;
    }
    setTimeout(poll, 50);
  })();
...
function startKakaoMap(container) {
  kakao.maps.load(function () {
    ...
    var pinchAccum = 0, pinchLock = false;
    container.addEventListener("wheel", function (e) {
      ...
      if (pinchAccum > 0) map.setLevel(map.getLevel() + 1, { anchor: mousePos });
      else map.setLevel(map.getLevel() - 1, { anchor: mousePos });
      pinchAccum = 0;
      pinchLock = true;
      setTimeout(function () { pinchLock = false; }, 90);
    }, { passive: false });
    ...
    if (clusterEl) {
      ...
      clusterEl.addEventListener("animationend", function () { clearTimeout(settleFallback); settleMap(); }, { once: true });
    } else { setTimeout(settleMap, 700); }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  renderFavoritesSidebar();
  setInitialStop().then(function (ok) { if (!ok) loadBusArrivals(); });
  ...
  mtabButtons.forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      mtabButtons.forEach(function (b) { b.classList.toggle("active", b === btn); });
      if (tab === "map" && window.__map) {
        setTimeout(function () { window.__map.relayout(); }, 60);
      }
    });
  });
});
